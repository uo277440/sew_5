...
class Restaurante {
    ...
    public function exportarCSV() {
        ...
        $this->exportar("clientesExportado.csv", "SELECT * FROM clientes");
        $this->exportar("empleadosExportado.csv", "SELECT * FROM empleados");
        $this->exportar("menusExportado.csv", "SELECT * FROM menus");
        $this->exportar("mesasExportado.csv", "SELECT * FROM mesas");
        $this->exportar("reservasExportado.csv", "SELECT * FROM reservas");
    
        $this->db->close();
    }
    
    protected function exportar($nombreFichero, $consultaSQL) {
        $fichero = fopen($nombreFichero, "w");
    
        $resultado = $this->db->query($consultaSQL);
        ...
        if ($resultado) {
            ...
            fclose($fichero);
            $contenidoCSV = file_get_contents($nombreFichero);
    ...
    AND NOT EXISTS (
        ...
        AND r.fecha = ?
        AND TIMEDIFF(r.hora, ?) BETWEEN '00:00:00' AND '01:00:00'
    )
    LIMIT 1";

    $stmt = $this->db->prepare($consultaDisponibilidad);
    $stmt->bind_param("iss", $capacidad, $fecha, $hora);
    $stmt->execute();
...
?>
    ...
    <form action="#" method="post">
        <input type="submit" value="Exportar datos a CSV" name="exportarCSV"/>
    </form>
